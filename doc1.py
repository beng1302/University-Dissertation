...
def random_uniform_sphere(N): # N random points on a unit sphere using a uniform distribution
	points=[]
	for i in range(0,N):
...
def random_gaussian_sphere(N,theta,phi,variance): # N random points on a unit sphere centered on a given point using the gaussian distribution
	points = []
	for i in range(0,N):
...
def local_monte_carlo_metropolis(points,iterations,temperature,type,variance): # Applies the Metropolis algorithm to a set of points
	system_energy = 0
	...
	for n in range(0,iterations):
		...
		if type=="uniform": # Generates the compared point by a uniform random distribution
			random_point = random_uniform_sphere(1)[0]
		elif type=="gaussian": # Generates the compared point by a local gaussian distribution centered on the chosen existing point
...
def pointplot(points): # Displays a set of points in 3D
	# Draws a sphere
...
def convexhull(points): # Creates a convex hull for a set of points
	pa = np.asarray(points)
	hull = ConvexHull(pa)
	ax.scatter(pa[:,0],pa[:,1],pa[:,2],s=10,color='k',alpha=1)
	for i in hull.simplices:
		verts = [zip(pa[i,0],pa[i,1],pa[i,2])]
		poly = Poly3DCollection(verts)
		poly.set_facecolors((1,0.9,0.9,0.7))
		poly.set_edgecolors((1,0,0,0.5))
		ax.add_collection3d(poly)
	
def main(): # Calls the desired functions and displays them
	iterations = 1000000
	temperature = 5*10**16
	type = "gaussian"
	variance = 0.00001
	rus = random_uniform_sphere(20)
	lmcm = local_monte_carlo_metropolis(rus,iterations,temperature,type,variance)
	#pointplot(lmcm)
	convexhull(lmcm)
	plt.show()
