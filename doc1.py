...
def random_uniform_sphere(N):
	"""N random points on a unit sphere using a uniform distribution"""
	points = []
	for i in range(0, N):
...
def random_gaussian_sphere(N, theta, phi, variance):
	"""N random points on a unit sphere centered on a given point using the gaussian distribution"""
	points = []
	for i in range(0, N):
...
def local_monte_carlo_metropolis(points, iterations, temperature, type, variance):
